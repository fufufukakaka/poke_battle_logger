...
first_in_battle_count as (
	select your_pokemon_1 as pokemon_name,
		count(1) as counts
	from target_battle_summary
	group by your_pokemon_1
),
second_in_battle_count as (
	select your_pokemon_2 as pokemon_name,
		count(1) as counts
	from target_battle_summary
	group by your_pokemon_2
),
third_in_battle_count as (
	select your_pokemon_3 as pokemon_name,
		count(1) as counts
	from target_battle_summary
	group by your_pokemon_3
),
...
unions_in_battle_win_count as (
	select *
	from first_in_battle_win_count
	union all
	select *
	from second_in_battle_win_count
	union all
	select *
	from third_in_battle_win_count
),
in_battle_win_count as (
	select pokemon_name,
		sum(counts) as counts
	from unions_in_battle_win_count
	group by pokemon_name
),
...
joins as (
	select in_team_count.pokemon_name,
		...
		coalesce(in_battle_win_count.counts, 0) as in_battle_win_count,
...
select pokemon_name,
	...
	coalesce(
		in_battle_win_count * 1.0 / NULLIF(in_battle_count, 0),
		0.0
	) as in_battle_win_rate,
	coalesce(

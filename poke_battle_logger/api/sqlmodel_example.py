...
from poke_battle_logger.models import Battle, BattleSummary, Trainer, get_session
...
async def check_trainer_exists_v2(
    trainer_id: str, session: Session = Depends(get_db_session)
) -> bool:
...
async def get_recent_battles_v2(
    trainer_id: str, limit: int = 5, session: Session = Depends(get_db_session)
...
async def create_trainer_v2(
    trainer_id: str, email: str, session: Session = Depends(get_db_session)
) -> Dict[str, str]:
...
async def get_recent_battle_summary_v2(
    ...
    if win_summaries:
        ...
        latest_win_pokemon = (
            latest_win.opponent_pokemon_1
        )  # Pick first opponent pokemon
    ...
    if lose_summaries:
        ...
        opponents = [
            latest_lose.opponent_pokemon_1,
            latest_lose.opponent_pokemon_2,
            latest_lose.opponent_pokemon_3,
        ]
